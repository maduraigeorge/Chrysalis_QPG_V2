...
const PaperPreview: React.FC<Props> = ({ mode, metadata, sections, questions, selectedBankQuestionIds }) => {
  ...
  if (mode === AppMode.BANK) {
    ...
    return (
      <div className="p-6 bg-white max-w-[210mm] mx-auto text-slate-900 relative">
        ...
        <div className="space-y-4">
          {selected.map((q, i) => (
            <div key={q.id} className="flex flex-col gap-1.5 border-b border-slate-100 pb-3 last:border-0">
              <div className="flex gap-3">
                <span className="font-black text-slate-900 text-xs w-5 shrink-0">{i + 1}.</span>
                <div className="flex-1">
                  <p className="text-xs text-slate-800 leading-snug font-semibold mb-1">{cleanText(q.question_text)}</p>
                  {q.image_url && (
                    <div className="my-1 border border-slate-100 p-0.5 rounded-lg inline-block max-w-full">
                      <img src={q.image_url} className="max-h-40 object-contain" alt="Question illustration" />
                    </div>
                  )}
                  <div className="flex gap-3 text-[8px] font-black uppercase tracking-widest text-slate-400 pt-1">
              ...
              {q.answer_key && (
                <div className="ml-8 mt-0.5 bg-slate-50 p-1.5 rounded-md border border-slate-200">
                  <p className="text-[7px] font-black text-slate-500 uppercase mb-0.5">Key:</p>
        ...
        <div className="mt-6 text-center text-[8px] font-black text-slate-300 uppercase tracking-[0.4em] pb-4">
          --- End of Export ---
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 md:p-8 bg-white max-w-[210mm] mx-auto min-h-[297mm] text-slate-900 relative">
      {/* Header Section */}
      <div className="text-center mb-4">
        ...
        <div className="grid grid-cols-2 border border-slate-900 px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-left gap-y-0.5">
      ...
      {metadata.instructions && (
        <div className="mb-4 text-[9px] leading-tight border-b border-slate-200 pb-2 italic">
          <p className="font-black not-italic uppercase mb-0.5 tracking-widest text-slate-900">Instructions:</p>
      ...
      <div className="space-y-5">
        {sections.map((section) => (
          <div key={section.id} className="section-container">
            <div className="flex justify-between items-end border-b border-slate-900 pb-0.5 mb-3 mt-3">
                ...
                return (
                  <div key={q.id} className="flex gap-3 items-start">
                    ...
                    <div className="flex-1">
                      ...
                      {q.image_url && (
                        <div className="mt-2 mb-1 flex justify-center p-1 border border-slate-100 rounded-lg">
                          <img src={q.image_url} className="max-h-[220px] object-contain" alt="Academic illustration" />
                        </div>
                      )}
                    </div>
      ...
      <div className="mt-8 pt-4 border-t border-slate-900 text-center font-black uppercase tracking-[0.4em] text-slate-300 text-[8px]">
        --- End of Examination ---
      </div>
      ...
      <div className="page-break" style={{ pageBreakBefore: 'always' }}>
        <div className="text-center mb-4 mt-8">
           <h2 className="text-base font-black uppercase tracking-tight border-b border-slate-900 pb-1 inline-block">Answer Key</h2>
        ...
        <div className="space-y-4">
          {sections.map((section) => (
            <div key={section.id + '_ans'}>
              <h3 className="text-[9px] font-black uppercase tracking-widest bg-slate-50 px-3 py-1.5 mb-2 rounded text-slate-800 border border-slate-100">{section.name}</h3>
              <div className="grid grid-cols-1 gap-2 ml-3">
                {section.selectedQuestionIds.map((qid, qIdx) => {
                  const q = questions.find(item => item.id === qid);
                  if (!q) return null;
                  return (
                    <div key={q.id + '_key'} className="flex gap-2 items-start border-l-2 border-slate-100 pl-3 pb-0.5">
                       <span className="font-black text-[9px] text-slate-400 min-w-[14px]">{qIdx + 1}.</span>
                       <div className="flex-1">
                          <p className="text-[8px] font-bold text-slate-300 uppercase tracking-tighter mb-0.5 line-clamp-1 italic">Ref: {cleanText(q.question_text).substring(0, 50)}...</p>
